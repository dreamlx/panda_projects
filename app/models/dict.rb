class Dict < ActiveRecord::Base
  ...
  def self.selected_client_statuses
  ...
  def self.selected_revenues
    where(category: 'revenue_type').order("code").map {|p| [ "#{p.code} || #{p.title}", p.id ] }
  end

  def self.selected_risks
    where(category: 'risk').order("code").map {|p| [ "#{p.code} || #{p.title}", p.id ] }
  end

  def self.selected_statuses
    where(category: 'prj_status').order("code").map {|p| [ "#{p.code} || #{p.title}", p.id ] }
  end
end
